...
excel_path = 'database/' + str(current_date) + '_midjouney.xlsx'
excel_path_2 = 'database/' + str(current_date) + '_midjouney-vjp.xlsx'
rar_path = ''
password_list = []
...
def vjp_df(df_a):
    if len(df_a) > 1:
        keywords = ['canva','netflix', 'midjourney' ,'amazon']
...
def core_process():
    ...
    if rar_have_password(rar_path):
        ...
        df_vjp = vjp_df(df_a)       
        collected_vjp = append_data_to_excel(excel_path_2, df_vjp)
    messagebox.showwarning("Alert", "Collected " + str(collected_vjp) + " account ")
